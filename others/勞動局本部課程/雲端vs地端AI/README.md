...
**本單元最終要產出的文件**：**會議記錄**、**決議事項**、**待辦事項**（須執行之具體工作）、**追蹤事項**（須列管或後續確認者）。以下流程以 [`手動合併_逐字稿.srt`](./資料來源/手動合併_逐字稿.srt)（或你自行合併之單檔）為唯一依據，**僅依逐字稿**，勿臆測未出現之內容。
...
兩段錄音各轉成一支 SRT 後，於編輯器**手動併成單一檔**（銜接與時間軸自行調整）。本倉庫範例為 [`手動合併_逐字稿.srt`](./資料來源/手動合併_逐字稿.srt)；產出**會議記錄、決議事項、待辦與追蹤事項**時，請以該檔（或你的合併檔）為定稿依據。
...
2. 以 VS Code 等編輯器開啟兩檔，將第二段文字接到第一段之後；若需**單一合法 SRT**（序號與時間軸連續），在編輯器內調整。給 Ollama 撰寫**會議記錄、決議、待辦與追蹤事項**時，重點是**內容完整、銜接合理**（步驟 3 會請模型略過時間碼、只依台詞）。
3. 存成你的合併檔，或對照本範例：[`手動合併_逐字稿.srt`](./資料來源/手動合併_逐字稿.srt)。

### 3. 用 Ollama `gpt-oss:20b` 產出會議記錄、決議／待辦／追蹤事項
...
3. 產出請含 **會議記錄**、**決議事項**、**待辦事項**、**追蹤事項** 四區；**僅依貼上之逐字稿**，勿臆測「上一場次」或其他未載明事項。下方提示詞已請模型略過 SRT 時間碼、只依台詞撰寫。

**會議文件用提示詞（貼入 `gpt-oss:20b`）**
