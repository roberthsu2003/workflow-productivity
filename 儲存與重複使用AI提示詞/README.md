...
| **Instructions** | **專案指示**＝此專案的系統提示詞（對應占位：**Add instructions to tailor Claude's responses.**）。 | 請貼上下方摺疊區的 **ROSES 全文**（週報整理助手專用，亦見「✏️ 小練習（Claude Projects）：週報整理助手」）。 |
...
**Instructions（專案指示）**：請到上方「**第二個畫面**」→ 摺疊區「**Instructions 欄建議全文**」，全選複製後貼入 **Add instructions**。
...
可從下列情境**擇一**發想（亦可改寫成與自己課業／工作相關的主題）：

| 情境 | 說明 |
|------|------|
| **📋 會議記錄助手** | 將口語化內容整理成正式會議紀錄。可搭配 [會議記錄原始檔](./會議記錄原始檔) 測試。 |
| **📝 週報整理助手** | 依範本與每日紀錄彙整週報。**以 Claude Projects 撰寫的完整步驟、Files／Instructions 與可複製 ROSES**，見「**三、Claude 的 Projects**」→「**第二個畫面**」與「**✏️ 小練習（Claude Projects）：週報整理助手**」。若改用 Gem／GPTs，請自行將「專案 Files／Instructions」對應改為「相關資訊／使用說明」或「知識庫／指令」。 |
