...
## O – 任務目標
每當使用者上傳 Excel 業績報表，請依讀取到的數據產出**一份**分析報告；**每次**都必須依序完成「輸出格式」中的**四個區塊**，不可省略。若欄位與下列假設不同，先以資料實際內容為準，仍須維持四區塊架構。
...
- **固定輸出格式**：建議你每次都維持相同的 Markdown 結構（例如四大區塊），體會「結構化輸出」的便利與威力——這將幫助你在未來自動整理或比對多份報表時省下大量時間。
- **異常偵測門檻可調整**：你可以將判斷「單月業績明顯低於平均」的門檻（原本設定 20%）改為 10% 或 30%，觀察結果變化，探索貼近實務需求的判準。例如將下列 prompt 內容：
  ```
  單月業績明顯低於該人員自身平均（低於 20% 以上）
  ```
  改成
  ```
  單月業績明顯低於該人員自身平均（低於 10% 以上）
  ```
  或
  ```
  單月業績明顯低於該人員自身平均（低於 30% 以上）
  ```

- **語氣規範可調整**：你可以實驗將「語氣正式、客觀」改為「親切、口語風格」，讓 AI 嘗試用更生活化的表達。這讓你體會同樣的數據，換個語氣就能應對不同溝通場景。例如將 prompt 裡
  ```
  語氣正式、客觀，適合對上呈報或製作會議報告
  ```
  改成
  ```
  語氣親切、口語化，像朋友分享發現一樣～
...
- **預想邊界情況、寫下注意事項**  
  好的 Prompt 應該考慮到各種「特殊情況」，像是資料缺漏、格式異常、無法判讀等等，明確要求 AI 告知原因，不要亂猜數字或編造內容。你可以在 Instructions 增加：「若遇格式不符、資料有缺，請明確說明缺失，不要臆造內容。」

---

> ☑️ **小提醒：請實際嘗試**調整以上參數，觀察 AI 輸出風格、判斷口徑的變化，思考哪一種寫法最適合你的真實工作場景。
...
請準備**一份可去識別化**的**真實**或**教學用**業績／銷售 **.xlsx**，依上表完成專案設定後，在對話中**完整跑過一次**：上傳檔案 → 確認 Claude 能讀取欄位 → 對照輸出是否含**趨勢、異常、摘要與建議**。若 xlsx skill 未開啟或無法使用，請改以「匯出 CSV 再貼上」等替代流程，並在 Instructions 中註明你的實際作法。
