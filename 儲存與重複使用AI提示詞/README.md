...
2. **📝 週報整理助手**：依週報範本與週一至週五工作記錄，彙整成結構化週報（可搭配 [週報整理助手原始檔](./週報整理助手原始檔)）。
3. **📊 數據摘要助手**：將複雜的報表數字整理成主管容易閱讀的 3～5 點重點摘要。

---

**主題背景**（以「週報整理助手」為例）：你要建立一個「**週報整理助手**」，將當週零散工作紀錄對照 [週報整理助手原始檔](./週報整理助手原始檔) 中的 **`週報範本.docx`** 結構，彙整為一則可交件的週報；來源為 **`當週工作記錄_週一.docx`**～**`週五.docx`**（可上傳知識庫或貼上全文）。若使用者尚未提供範本或任一日紀錄，請先詢問或請對方補齊後再整理。
...
## O – 任務目標
依我提供的「週報範本」結構，整合「週一至週五」工作記錄，產出**一則完整週報**。內容需忠實於我提供的紀錄，不臆測未提及的進度或數字。

## S – 執行步驟
1. **輸入檢查**：若尚未收到週報範本要點或任一日之工作紀錄，請簡短說明缺哪一部分，並請我補上（可上傳檔案或貼上文字）後再繼續。
2. 對照範本：辨識範本中的標題、欄位、表格或區塊順序。
3. 依日期（週一～週五）梳理各日重點，合併重複項、補上過渡句，使全文連貫。
4. 將內容**對應填入**範本結構；若某欄位在紀錄中無資料，標示「本週無」或「待補充」，不要虛構。
5. 產出前自行檢查：是否涵蓋範本必填區塊、用語是否一致為職場書面語。

## E – 輸出格式
使用 Markdown 輸出，**章節標題與順序**盡量與我提供的週報範本一致，例如：
