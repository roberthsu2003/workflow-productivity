...
| **Instructions** | **專案指示**＝此專案的系統提示詞（對應占位：**Add instructions to tailor Claude's responses.**）。 | 請貼上下方摺疊區的 **ROSES 全文**（與「🚀 綜合練習」建議 Prompt 相同）。 |
...
<details>
<summary>點擊展開：Instructions 欄建議全文（請全選複製後貼入 Add instructions）</summary>
...
與「🚀 綜合練習」內「💬 建議 Prompt」內容相同，擇一處複製即可。
...
2. **📝 週報整理助手**：**週報範本**預先放在專案（或知識庫）內，再依當週週一至週五工作記錄彙整成週報（可搭配 [週報整理助手原始檔](./週報整理助手原始檔)）。
3. **📊 數據摘要助手**：將複雜的報表數字整理成主管容易閱讀的 3～5 點重點摘要。

---

**主題背景**（以「週報整理助手」為例）：你要建立一個「**週報整理助手**」。**`週報範本.docx`** 已預先置於專案／知識庫（不必每週重複提供）；每次整理時只需提供當週 **`當週工作記錄_週一.docx`**～**`週五.docx`**（可上傳或貼上全文，檔案見 [週報整理助手原始檔](./週報整理助手原始檔)）。若專案內尚無範本或缺少任一日紀錄，請先說明缺件並請對方補齊後再整理。

> 若你已在「**三、Claude 的 Projects**」→「**第二個畫面**」的 **Instructions** 貼過下列全文，練習時無需再重複貼上。
