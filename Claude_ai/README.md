...
> **📂 [進入主題筆記：Local_MCP](./Local_MCP/README.md)**（設定檔修改、本機檔案存取、Debug 範例）

**Local MCP** 是執行在您**本機電腦**上的伺服器，通常透過修改 `claude_desktop_config.json` 設定檔來啟用。它賦予 Claude 存取本機資源的能力，例如讀取特定目錄的檔案、執行終端機指令或連結本機資料庫。與 Connectors 不同，它不需要 OAuth，但需要您在本地端配置環境（如安裝 Node.js 或 Python）。
...
> **📂 [進入主題筆記：Settings](./Settings/README.md)**（快捷鍵、電腦操作模式、能力開關）

**Settings** 是管理帳戶、偏好與功能的核心。在此您可以設定**快捷啟動鍵**、開啟 **Computer use** (Beta) 讓 Claude 操作電腦、或在 **Capabilities** 中啟用 **Artifacts** 與 **Analysis Tool**。這也是進行 **Connectors** 與 **Developer** (Local MCP) 配置的入口。
