...
請幫我檢查有沒有違反公司報銷規定？若有不合規或資料缺漏的請明確列出並告訴我怎麼補件，最後幫我產出正式的 Excel 報銷請款單。
```

> [!TIP]
> **💡 一般使用者無須知道 Skill 的內部路徑！**  
> 使用者不需要在 Prompt 中指名 `references/expense-policy.md` 或 `templates/`。Skill 已經在其 `SKILL.md` 中被賦予了自動查找規章、載入 Excel 樣板與嵌入 Logo 的 SOP。使用者只需用日常商務口語提出需求即可！
...
#### 📥 測試步驟與 Prompt（純圖片）：
1. 將上述 **5 張圖片全部拖入對話框**。
2. 搭配輸入以下日常指令（無須提及任何內部程式碼或樣板路徑）：

```text
這是我手邊的 5 張出差報銷單據圖檔，申請人為「業務部 陳大華 (David Chen)」。
請直接幫我辨識各張單據的內容並審查是否符合公司報銷規範。
若有超標、統編錯誤或缺漏事由請告訴我如何補件，最後幫我產出正式的 Excel 請款單。
```
...
請幫我把圖片辨識的項目和這兩筆文字合併審核，看看有沒有違規，並產出一份正式的 Excel 報銷單供我下載。
```
...
* **🔍 階段 1・財務合規性診斷與停止條件判定**  
  - 第 1 筆（計程車 NT$ 380）：`✅ 合規`（有攜帶樣品事由，未超額 NT$ 500）。
  - 第 2 筆（高鐵票 NT$ 700）：`✅ 合規`（標準車廂對號座，統編正確）。
  - 第 3 筆（Uber NT$ 560）：`⚠️ 需補件 / ❌ 超標`（超過 NT$ 500 上限，且**未填寫事由**，觸發停止邊界）。
  - 第 4 筆（牛排宴客 NT$ 2,800）：`✅ 合規`（商務宴客每人限額 NT$ 1,500，2 人上限 NT$ 3,000，申報 NT$ 2,800 合規）。
  - 第 5 筆（加班燒肉 NT$ 850）：`❌ 超標`（個人日常公務餐費上限為 NT$ 600，超額 NT$ 250 標註超標自負）。

* **🚨 階段 2・安全煞車與主動詢問（Human-in-the-Loop）**  
  面對第 3 筆資料缺漏，AI **不會自行編造事由**，而是主動向使用者發出專業提示：
  > 「⚠️ **第 3 筆 Uber 車資 (NT$ 560)** 目前缺少公務急迫事由，且超出單趟 NT$ 500 上限。  
  > 請問您希望：  
  > 1. **立即在此補充事由**（例如補上客戶名稱與急迫原因），重新計算並核銷？  
  > 2. **先暫停本階段**，等取得主管簽呈後再送審？  
  > 3. **以現有狀態先行產出 Excel 請款單**（標註 ⚠️ 需補件，超額 60 元列自負）？」

* **🐍 階段 3・Python 自動化處理與報表交付**  
  確認後呼叫 Code Execution，以 `openpyxl` 載入 `templates/expense-report-template.xlsx`：
  - 填入申請人姓名「陳大華 (David Chen)」、部門「業務部」。
  - 在 Cell A1 嵌入高清晰 `assets/company-logo.jpeg` 圖檔。
  - 將各筆明細寫入試算表，狀態欄搭配清楚圖示標註。
