...
這個 Skill 會將您隨手寫下的雜亂客戶專案需求、報價項目與折讓條件，重新進行金額計算，並套用公司標準報價單樣板，產出格式嚴謹、專業的商務報價單。
...
├── SKILL.md
└── templates/
    └── quotation-template.md  # 儲存報價單標準 Markdown 樣板
```

## 🛠️ 安裝與使用方式

### 💡 方式 A：使用內建 `/skill-creator` 技能（自動建立）
1. 先將 [quotation-template.md](./templates/quotation-template.md) 檔案上傳至 Claude 對話中。
2. 直接下指令：
   > 「`我想建立一個客戶報價單 Skill。請參考我剛才上傳的樣板檔案格式，使用 /skill-creator 幫我建立包含 templates 資料夾的 Skill。`」

### ✍️ 方式 B：手動複製檔案（手動建立）
1. 在電腦中建立新資料夾 `Office_Workflow_03_Customer_Quotation`，並在其中建立名為 `templates` 的子資料夾。
2. 複製此資料夾下的 [SKILL.md](./SKILL.md) 內容儲存於根目錄；將 [quotation-template.md](./templates/quotation-template.md) 內容儲存至 `templates/` 目錄中。
...
**預期效果：**
Claude 將會自動啟用該 Skill，並依據 `templates/quotation-template.md` 的結構，將上述內容進行金額與折讓計算後，整理成格式精準的專案服務報價單。
