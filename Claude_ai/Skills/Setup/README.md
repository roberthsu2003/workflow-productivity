...
> 
> **🧪 實驗一：探測已啟用的核心（Public）、範例（Examples）與自訂（User）技能**
...
> ## Constraint
> - 語言：繁體中文。
> - 格式：以表格呈現 Public 核心技能名稱與功能說明，並列出 examples 範例技能總個數與名稱清單。
> ```
> 
> 3. **驗證與診斷**：
>    - **順利回覆（如：Public 核心技能有 9 個，Examples 範例技能有 24 個，User 自訂技能有 5 個）** 🎉：代表您的 Claude 帳號具備完整的沙盒磁碟存取與程式碼執行能力，已成功啟用！
...
> 請引導學生嘗試以下三個簡單的整合型範例，複製下方程式碼區塊內的 Prompt 給 Claude，快速驗證「核心技能」與「Examples 範例模板」是否能成功協同運作：
> 
...
| 比較維度 | 📂 `public/`（自動觸發） | 📂 `examples/`（需明確指名） | 📂 `user/`（自訂技能） |
| :--- | :--- | :--- | :--- |
| **生動比喻** | 公司給你的**正式工作手冊**（一入職就發在桌上） | 放在**公司檔案室裡的歷史參考檔案**（要自己去查或被交代才看） | 你自己的**私人筆記與獨門絕活**（放在抽屜裡隨時拿出來用） |
| **存放路徑** | `/mnt/skills/public/` | `/mnt/skills/examples/` | `/mnt/skills/user/` |
| **Claude 的認知** | 預設就知道（寫在系統提示詞中） | 預設不知道（但可用磁碟讀取工具讀取） | 預設不知道，但可隨時查詢與讀取 |
| **是否需提醒？** | **不需要**，直接要求「幫我做一個簡報」即可觸發 | **需要**，必須在對話中明確說「使用 `theme-factory` 技能」 | **需要**，可以問「列出我的自訂技能有哪些？」或「檢查 `/mnt/skills/user/` 目錄」 |
| **主要功能** | 生成簡報、試算表、Word 等實體檔案 | 主題套用、畫布排版、寫作風格學習等進階功能 | 使用者自行開發或上傳的自訂技能（內含 `SKILL.md`） |
...
## 📂 3. 自訂技能庫（📂 `/mnt/skills/user/`）

除了系統預載的核心技能與範例技能外，Claude 還支援由使用者自行定義與擴充的**自訂技能 (User Skills)**。

### 💡 查詢與使用自訂技能
當您在具備自訂技能的環境（例如專案工作區或特定 CLI 代理環境）中，您可以直接問 Claude 來探索或使用這些技能：
*   **Prompt 範例**：
    *   *「列出我的自訂技能有哪些？」*
    *   *「檢查 `/mnt/skills/user/` 目錄，看看有什麼自訂技能」*
*   **系統反應**：Claude 會主動到 `/mnt/skills/user/` 目錄下讀取您所擺放的技能資料夾與 `SKILL.md`，並將其載入為本次對話的可用技能。

> 💡 *說明：本教材後續的「自訂 Skills 四階演進路徑與建立方式」所教您製作的 `SKILL.md` 與專屬工作流，即是存放在此處以供調用。*
