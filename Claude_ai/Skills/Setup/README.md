...
> | :--- | :--- | :--- |
> | **A. 順利產生實體簡報檔並提供下載連結** 🎉 | 帳號為 **Pro / Max / Team / Enterprise** | 內建文件技能已自動在雲端生效，**不需手動下載或安裝**即可直接呼叫使用。 |
> | **B. 出現「您的方案不支援 Code execution」或提示需升級 Pro** ⚠️ | 帳號為 **Free（免費版）** | 免費版無法執行官方內建文件 Skills（無法在雲端運算實體檔）。教學重點請引導該生走 **「手動建立/下載自訂 Skill (內含 SKILL.md)，並點擊『+』上傳 .zip 檔」** 的自訂技能路線（免費版依然支援此功能）。 |
