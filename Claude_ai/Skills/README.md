...
* **延伸實戰練習（依授課對象選用）**：
  ...
  * 🌿 **綠能科研情境（工研院綠能所 / 科研機構專班）**：請參閱 [工研院綠能所公文與產學回函修飾專家（第一階：模仿者篇）](#itri-imitator)。
...
* **延伸實戰練習（依授課對象選用）**：
  ...
  * 🌿 **綠能科研情境（工研院綠能所 / 科研機構專班）**：請參閱 [工研院綠能所專案技術摘要與規格評核專家（第二階：創作者篇）](#itri-creator)。
...
## 🌿 垂直產業專案：工研院綠能與環境研究所（ITRI GEL）科研專案自訂技能

> **適用對象**：專門為**工研院綠能與環境研究所（ITRI GEL）各研發組室、科研計畫主持人與專案工程師**設計的專屬自訂技能。  
> 針對政府公文往返、產學合作洽詢回函修飾（第一階：模仿者），以及對齊國家 2050 淨零路徑、TRL 1~9 與 CNS/IEC 安規之專案技術摘要與規格合規評核（第二階：創作者）。

<div id="itri-imitator"></div>

### 1. 🏛️ [工研院公文、研發簽呈與產學合作回覆信函修飾專家（第一階：模仿者篇）](./ITRI_Imitator/README.md)
* **適用階段**：第一階：模仿者 (Imitator) —— 單獨建立 `SKILL.md`
* **核心涵蓋**：
  - 政府公文三段式結構（主旨、說明、辦法）自動轉換。
  - 民間企業產學技術洽詢回函，熱情展現研發能量同時嚴格建立營業秘密與 NDA 保密防線。
  - 科技法人專業分寸拿捏，自動消弭「絕對保證」等過度承諾字眼，附帶秘書室叮嚀。
* **實戰技術**：單一檔案 `SKILL.md`（定義 `Role`, `Task`, `Constraint`, `Format` 與法人分寸守則）。

<div id="itri-creator"></div>

### 2. 📑 [綠能專案技術摘要與規格評核專家（第二階：創作者篇）](./ITRI_Creator/README.md)
* **適用階段**：第二階：創作者 (Creator) —— `SKILL.md` + `references/` 規章庫 + `templates/` 標準樣板
* **核心涵蓋**：
  - **模式 A（專案技術摘要）**：輸入雜亂筆記，自動對照 TRL 成熟度與減碳係數公式，套用標準五大章節報告樣板。
  - **模式 B（規格合規評核）**：輸入儲能貨櫃或巨型風機規格書，自動比對 CNS 62933、IEC 61400 Class T 抗颱與 UL 9540A 延燒測試，抓出重大違規紅旗（Red Flags）。
* **實戰技術**：智慧雙路由切換 + `references/`（TRL 評估規章、綠能安全標準）+ `templates/`（技術摘要與合規清單樣板）。
