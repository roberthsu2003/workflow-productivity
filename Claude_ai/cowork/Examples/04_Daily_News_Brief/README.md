...
3. **啟用雲端定時排程 (Scheduled Tasks)**：
   > ⚠️ **新手常見問題：為什麼沒看到「Schedule this task」按鈕或 `/schedule` 指令？**  
   > - **介面位置**：Claude 目前已將排程功能整合至**左側邊欄的「Scheduled」專區**（位於 Projects / Artifacts 下方）。  
   > - **本機資料夾限制**：若您當前會話掛載了本機資料夾（如 `sample_files/`），因為雲端伺服器在您關機時無法存取您電腦的實體硬碟，故此模式下介面右上角通常不會顯示一鍵轉排程按鈕。

   **建議操作方式（二選一）：**
   - **方式 A（從左側「Scheduled」直接建立・最推薦）**：
     1. 點擊左側導覽列的 **`Scheduled`**。
     2. 點擊右上角 **`New task`**（選擇 `Set up manually`）。
     3. 頻率設為 **`Every weekday`（每工作日）08:00 AM**。
     4. 將上述 RTCCF Prompt 貼入（關鍵字直接寫在 Prompt 中，免讀取本機檔案），點選儲存完成！
   - **方式 B（對話中直接下達指令）**：
     在對話輸入框中直接輸入自然語言交代：
...
>    - **功能入口改版**：請直接查看 **Claude 左側導覽列的「Scheduled」** 標籤，所有排程都在此管理與新增。
>    - **雲端排程與本機檔案互斥**：純雲端排程是在 Anthropic 雲端伺服器執行（才能實現電腦關機也能跑）。若會話綁定了本機實體資料夾（如 `sample_files`），系統無法將「存取本地端檔案」的任務直接轉為離線雲端排程。設定排程時，建議將關鍵字與格式規則直接嵌在 Prompt 中，產出直接儲存為 Claude Artifact 或同步至 Google Drive。
> 2. **什麼樣的排程需要開機？什麼不需要？**  
