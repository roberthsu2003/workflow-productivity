...
本實作以「**星橋科技行政特助**」為例，透過 Before / After 的直接對照，讓你在 10 分鐘內親身感受 Claude Projects 如何徹底解決「每次開新對話都要重教背景與術語」的痛點。

---

### 📥 步驟 0：下載實測練習檔案

請先下載以下兩份課堂模擬檔案（點擊連結即可另存）：

| 檔案名稱 | 類型 | 用途說明 |
| :--- | :---: | :--- |
| 📄 [**內部術語對照表.md**](./Examples/01_Office_Administration/sample_files/內部術語對照表.md) | 知識庫檔案 | 星橋科技內部黑話與正式名稱對照（稍後上傳至 Knowledge） |
| 📝 [**會議逐字稿_產品週會.md**](./Examples/01_Office_Administration/sample_files/會議逐字稿_產品週會.md) | 測試輸入檔 | 充滿內部黑話的真實會議記錄（稍後作為提問素材） |

---

### ❌ 步驟 1：在「一般對話」測試（Before）

先體驗在**未建立專案**的情況下處理業務：

1. 開啟一個全新的普通 Claude 對話（一般 Chat）。
2. 貼上 [會議逐字稿_產品週會.md](./Examples/01_Office_Administration/sample_files/會議逐字稿_產品週會.md) 全文，並在最後附上指令：
   ```text
   幫我把這份逐字稿整理成會議紀錄，列出決議事項與待辦清單。
   ```

> 📉 **執行結果（痛點體驗）**：  
> Claude 只能按字面胡亂猜測——它完全不知道「紅單」是最高優先級障礙單、「過橋」是正式環境部署。你必須花費大量時間手動修正術語。

---

### ⚙️ 步驟 2：三步驟建立你的第一個 Project

現在，我們將「行為準則」與「專屬知識」注入專案中：

1. **建立專案**：
   - 點擊左側選單 **Projects** ➔ 點擊 **New project**。
   - 專案名稱：`星橋科技行政特助`
   - 專案目標：`整理會議紀錄與內部術語標準化`
2. **設定行為指令（Instructions）**：
   在 **Set project instructions** 欄位貼入以下內容：
   ```text
   你是星橋科技的資深行政特助。

   整理文件時，先比對專案知識庫中的「內部術語對照表」，將所有內部術語標註正式名稱（例如：紅單/最高優先級障礙單）。
   ...
   - 待辦清單（含負責人與期限）
   ```
3. **上傳專案知識庫（Project Knowledge）**：
   在 **Project Knowledge** 區塊點擊 **Add content** ➔ 上傳剛剛下載的 [內部術語對照表.md](./Examples/01_Office_Administration/sample_files/內部術語對照表.md)。

---

### ✅ 步驟 3：在 Project 裡重做同一件事（After）

1. 在該專案頁面內點擊開啟新對話。
2. 貼上**同一份逐字稿**，輸入**完全相同的指令**：
   ```text
   幫我把這份逐字稿整理成會議紀錄，列出決議事項與待辦清單。
   ```

> 🚀 **執行結果（效果見證）**：  
> Claude 自動精準轉譯：「收到兩張紅單（最高優先級障礙單，需 24 小時內回應）」、「需經 CAB 核准後過橋（部署至正式環境）」——**你一個字都不需要多作解釋，輸出即達專業交付標準！**

---

### ⚡ 步驟 4：跨對話長期記憶驗證（Killer Feature）

在該專案內**再開一個全新對話**，這次**不貼任何文件**，直接提問：
...
> 🎯 **執行結果**：  
> Claude 秒回：「90 天新人入職培訓計畫，T0 紅單為 24 小時內回應」。  
> **知識只需上傳一次，該專案底下的所有新對話永久自動繼承，再也不必反覆複製貼上背景說明！**
