...
1. **獨立面板即時預覽**：對話旁側欄獨立畫布，HTML/CSS/JS、React、Tailwind CSS 等視覺設計即時渲染。
2. **支援多種內容格式**：涵蓋 Markdown 文件、互動網頁、Python 程式碼、動態圖表（Recharts）、流程圖（Mermaid）與 SVG 向量圖。
3. **介面隔離**：主要對話區專注於 Prompt 邏輯討論，右側專注於成品呈現，互不干擾。
4. **反覆修改與版本歷史**：同畫布持續疊代更新（V1、V2...），隨時能一鍵回溯切換舊版本，不怕改壞。
5. **單次對話支援多檔案**：一段對話中可陸續產出多個獨立的 Artifacts（如文案、流程圖、網頁分開存放）。
6. **[AI 驅動成品 (Claude in Claude)](#feature-6)**：*（進階）* 免 API Key，讓產出的網頁小工具內部也能直接調用 AI 即時動腦生成。 [查看詳細說明與費用機制 ➔](#feature-6)
7. **[持久化資料儲存 (localStorage)](#feature-7)**：*（進階）* 讓網頁擁有「記憶力」，關閉重開或重新整理後資料不消失。 [查看詳細說明與語法 ➔](#feature-7)
8. **[可串接外部服務 (Connectors & MCP)](#feature-8)**：*（進階）* 串接 Google Drive、Gmail 等，讓成品讀取真實檔案或一鍵回存雲端。 [查看詳細說明與情境 ➔](#feature-8)
9. **[公開分享與二次修改 (Remix)](#feature-9)**：*（進階）* 一鍵產生免登入公開展示網址；支援複製程式碼到新對話進行安全二次創作。 [查看詳細說明與SOP ➔](#feature-9)
10. **自動與手動判斷**：Claude 會依內容長度與重複使用性自動開啟畫布，亦可在 Prompt 主動要求「幫我做成 Artifact」。
...
## 🔍 進階特性深度解析（AI大腦、資料儲存、外部串接、Remix）

針對進階使用者與職場實戰需求，以下為四項關鍵功能的詳細運作原理與實作技巧：

---

<a id="feature-6"></a>
### 6. AI 驅動的成品 (Claude in Claude / 讓成品裡內建 AI 大腦)
...
[↑ 回到核心功能速覽](#features-summary)

---

<a id="feature-7"></a>
### 7. 持久化資料儲存 (Persistent Storage / 讓成品擁有「記憶力」)

- **痛點是什麼？（如果沒有持久化）**：
  一般的網頁小工具（如待辦事項表、記帳清單），你在裡面辛辛苦苦輸入了 10 筆資料，只要**按 F5 重新整理、切換網頁或關閉瀏覽器，剛才打的所有資料瞬間蒸發、變成一片空白**，每次打開都要從頭來過。
- **持久化的白話定義**：
  就是**「替網頁加上記憶力」**！就算你把網頁關掉、電腦關機，明天再次點開網址，昨天輸入的所有紀錄、帳目明細、勾選進度或個人設定**依然完好存在**。
- **它是怎麼做到的？（最常見的實作方式）**：
  - 🔹 **瀏覽器本地儲存（`localStorage`，最簡單且最推薦）**：
    - **原理**：資料直接安全地保存在使用者自己的電腦或手機瀏覽器中，**完全不需要架設後端資料庫，也不用花錢買雲端伺服器**。
    - **如何跟 Claude 說（Prompt 範例）**：只要在對話中加一句：
      > *「請幫我使用 `localStorage` 來儲存使用者輸入的資料，確保重新整理頁面或重開瀏覽器後，資料依然不會消失，並提供一個『清空所有資料』的重置按鈕。」*
  - 🔹 **Claude 官方環境儲存機制**：在進階的 AI-powered 原型中，利用 Claude 內建的存取功能保留應用狀態。
- **職場實戰情境舉例**：
  - **情境 1：活動籌備檢核清單（Checklist）**：同仁勾選了「借用場地已完成」、「印製簡報已完成」，下次開會點開連結時，已勾選的進度一目了然，不用重新回想。
  - **情境 2：辦公室零用金／飲料訂購記帳**：每天輸入「買影印紙 $150」、「下午茶 $400」，系統自動累加並永久保留明細。
  - **情境 3：個人化介面偏好**：記住使用者選擇的「字體大小（大/中/小）」或「深色模式（Dark Mode）」，下次開啟時自動套用。
- ⚠️ **注意事項與邊界**：
  - **資料跟著設備走**：因為是存在自己的瀏覽器裡，如果在辦公室電腦輸入，回家用手機點開該連結，手機上會是空白的（彼此獨立）。
  - **清除快取會重置**：如果主動按了「清除瀏覽器歷史紀錄/快取」或使用「無痕模式」，儲存的資料會被清空。若要全公司跨設備多人即時同步，則需搭配外部服務串接（如 Google Sheets）。
...
<a id="feature-8"></a>
### 8. 可串接外部服務 (Connectors & MCP / 讓成品讀寫真實世界資料)

- **痛點是什麼？（如果沒有串接服務）**：
  以往請 AI 做報表或儀表板，裡面只能塞假資料（Mock Data），或者你必須自己手動把 Excel、PDF 下載到電腦，再手動上傳到對話框，改完後又要自己複製貼上存檔，流程非常繁瑣破碎。
- **串接服務的白話定義**：
  透過官方內建的 **Connectors（連接器目錄）** 或 **MCP（模型上下文協定）**，直接為 Claude 打通外部工具的橋樑（如 **Google Drive、Gmail、GitHub、Notion 等**），讓右側的 Artifact 能**直接讀取真實資料做成圖表，或將做好的成品一鍵存回雲端**！
- **它是怎麼運作的？（官方授權與安全機制）**：
  - **一鍵綁定授權**：在 Claude 個人設定（Settings > Connectors）中授權綁定您的 Google 帳號。
  - **安全確認機制（Approval-based）**：Claude 絕不會擅自亂動檔案；當小工具需要發送 Email 或寫入新檔案時，介面一定會跳出**「是否同意執行此動作？」**的確認按鈕，由您親自點擊同意後才會執行。
- **職場實戰三大應用情境**：
  - 🔹 **情境 1：Google Sheets 雲端試算表 ➔ 即時互動戰情室**：
    - **用法**：直接跟 Claude 說：*「請讀取我 Google Drive 中的『2026年Q1業績明細.xlsx』，在右側製作一個可按部門篩選的 HTML 動態圖表 Artifact。」*
    - **價值**：不用重新下載上傳，幾秒鐘內把密密麻麻的試算表格轉化為長官看得懂的互動戰情室。
  - 🔹 **情境 2：Gmail 郵件串 ➔ 專案待辦清單 Artifact**：
    - **用法**：要求 Claude：*「搜尋我 Gmail 中關於『機房升級專案』的近 5 封往來信件，整理出重點決議，並在右側產生一份 Markdown 專案進度追蹤表 Artifact。」*
  - 🔹 **情境 3：成品一鍵回存雲端硬碟（Google Docs）**：
    - **用法**：跟長官開會修改到 Version 3 定稿的企劃書，直接輸入：*「請把這個 Artifact 成品直接儲存一份到我 Google Drive 的『2026 企劃部』資料夾中。」*
- ⚠️ **安全性與隱私提醒**：
  - **不用擔心資料外洩給訪客**：當你將 Artifact 發布公開連結給同事或主管時，**外部人員絕對無法透過該連結窺探你的 Google Drive 私人檔案**；連接器權限僅綁定在您個人的登入環境中，資安防護嚴密。
...
<a id="feature-9"></a>
### 9. 分享與二次修改 (Publish & 手動 Remix 操作)

- **公開分享連結**：點擊右上角 **Publish** 可生成專屬網址，分享給其他人（**對方不需要 Claude 帳號**也能在瀏覽器中點開檢視與完整操作互動）。
- **❓ 常見疑問：畫面上以前的「Remix」按鈕去哪了？**
  - **官方最新現況**：Anthropic 官方在近期改版中已將一鍵「Remix」按鈕下線移除（*The 'Remix' button is no longer available*）。
  - **現在如何「Remix（在此基礎上做自己的版本）」？** 官方推薦的標準操作非常簡單：
    1. 打開該 Artifact 分享網址，點擊右上角 **「Code」** 分頁切換到程式碼檢視。
    2. 點擊右上角的 **「Copy Code」** 複製全部程式碼。
    3. 回到自己的 Claude.ai 開啟新對話貼上，並下達修改要求（例如：*「這是同事做好的網頁成品，請在此基礎上幫我改配色、並多加一欄備註。」*）。
    4. 這樣就能在自己的帳號裡產生全新的獨立版本，完全不會影響或覆蓋到原作者的原始成果！
...
5. **分享與二次修改（Remix 操作）**：示範點選發布分享連結功能，並示範觀看者如何透過「複製程式碼到新對話」進行二次創作與修改（Remix），說明彼此檔案各自獨立、互不影響的特性。
