...
7. **持久化資料儲存 (Persistent Storage / 讓成品擁有「記憶力」)**：*（進階功能）*
   - **痛點是什麼？（如果沒有持久化）**：
   ...
   - ⚠️ **注意事項與邊界**：
     ...
     - **清除快取會重置**：如果主動按了「清除瀏覽器歷史紀錄/快取」或使用「無痕模式」，儲存的資料會被清空。若要全公司跨設備多人即時同步，則需搭配第 8 點（串接 Google Drive/Sheets）。
8. **可串接外部服務 (Connectors & MCP / 讓成品讀寫真實世界資料)**：*（進階功能）*
   - **痛點是什麼？（如果沒有串接服務）**：
