...
### ⚙️ [01. Settings（環境準備、隱私與 2026 最新桌面端設定全解析）](./01_Settings/README.md) — Free / Plus
> 現代 AI 工作流中樞！解析 2026 最新桌面版設定四大核心維度：**個人**（Pets 狀態寵物、組態、使用量分析）、**整合**（內建獨立瀏覽器 Browser Use、Computer use 電腦操控、Appshots 前景視窗快照）、**程式設計**（Git Worktrees 平行工作樹、Environments 環境自動化、Hooks 生命週期掛勾）與**公務機密防護**（Data Controls 關閉模型訓練、2FA 雙因子驗證與密碼管理）。
