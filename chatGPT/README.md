...
| :--- | :---: | :---: | :---: | :--- |
| **旗艦模型選擇（GPT-4o）** | ✓（動態額度限制） | ✓（高用量額度） | ✓（無限制或極高額度） | [02_Chats](./02_Chats/README.md) |
| **深度思考推理（o1 / o3-mini）** | ✓（基礎額度） | ✓（高額度） | ✓（專屬額度） | [02_Chats](./02_Chats/README.md) |
...
> 💡 **教學與自學指引**：
> - **學生端**：註冊免費版（Free）即可完整實作 01 ~ 05 單元的所有基礎操作，並可體驗 Canvas 協作、Python 數據分析與 GPT-4o 多模態辨識。
...
| **對話與提示詞架構** | Chats（RTCCF 框架） | [02_Chats](./02_Chats/README.md) | ROSES 與 RTCCF 框架、GPT-4o vs o1/o3-mini 思考模型選用 |
...
### 🟢 [02. Chats（對話、思考模型與提示詞工程）](./02_Chats/README.md) — Free / Plus
> 了解 ChatGPT 現代模型梯隊：**GPT-4o**（日常多工多模態旗艦）、**o1 / o3-mini**（專精於數學、程式碼除錯與高難度邏輯推理的深度思考模型）。掌握職場必備 **ROSES 框架**（Role, Objective, Scenario, Expected Outcome, Steps），輕鬆應對複雜商務溝通。
