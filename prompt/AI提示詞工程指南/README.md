...
**快速填空（可貼在記事本重複使用）** — 以下提供 **YAML**（利於結構化、設定檔、API）與 **Markdown**（利於閱讀、直接貼入對話）兩種寫法，擇一即可。
...
【Constraints】限制：……（字數、語氣、禁止事項）

【Format】輸出請用……（例如：三段式 Email、含欄位之 Markdown 表格）
```
...
以下範例皆採 **Role + Task + Context + Constraints + Format**；每則含 **YAML** 與 **Markdown** 兩種語法，並以摺疊區呈現，方便複製與對照。
...
task: 撰寫一封給客戶的致歉 Email，說明訂單出貨延誤並提出補償。
context: |
  發信單位：華梵大學圖書館採購承辦；對象：三民書局（客戶端聯絡窗口）。
  事實：50 本圖書因供應商備料不足無法於原定日出貨；預計下週五可出貨。
  補償：提供九折優惠券。
constraints:
...
請用 **Role + Task + Context + Constraints + Format** 撰寫這封 Email 的 Prompt（五段都試著填滿）；可任選 YAML 或 Markdown 格式。
...
（請在 **context** 或【Context】區塊貼上你的實際數據。）
...
請用 **Role + Task + Context + Constraints + Format** 設計 Prompt（YAML 或 Markdown 擇一）；**Format** 請明確要求「表格＋優先級欄位」或「條列＋每類一句回覆方向」。
...
**版本 B（五要素版）：**  
完整寫出 **Role、Task、Context、Constraints、Format**（可參考上方範例 4 的 YAML／Markdown 結構）。
...
以下為**示意**：模擬「部門常見問答助理」貼在專案 Instructions 的寫法（實務上請替換成真實規章與聯絡窗口）。同樣提供 **YAML**（便於結構化儲存）與 **Markdown**（便於直接貼入 Instructions）。
...
任選一項你常重複請 AI 做的事（例如：每週部門週報格式、固定客訴回覆邏輯），先寫一版 **Role + Task + Context + Constraints + Format**，再改寫成 **ROSES**（特別補強 **Steps** 與「無法回答時」的規則）。
...
- 同一套內容可寫成 **YAML**（結構化、易於版本管理）或 **Markdown**（易讀、易貼入對話），依工具與習慣擇一即可。
