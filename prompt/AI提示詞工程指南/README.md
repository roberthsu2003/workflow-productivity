...
**快速填空（可貼在記事本重複使用）** — 以下提供 **自然語言**（口語一段話，最貼近日常對話習慣）與 **Markdown**（利於閱讀、直接貼入對話）兩種寫法，**擇一即可**；內容等價，差在形式。摺疊區順序為：**自然語言 → Markdown**。
...
以下範例皆採 **Role + Task + Context + Constraints + Format**；每則含 **自然語言** 與 **Markdown** 兩種寫法（摺疊區順序亦同），方便複製與對照。（習慣直接聊天的同學，可從第一個摺疊「自然語言」開始，再對照結構化寫法是否涵蓋五要素。）
...
請用 **Role + Task + Context + Constraints + Format** 撰寫這封 Email 的 Prompt（五段都試著填滿）；可任選 **自然語言**（一段話）或 **Markdown**。
...
請用 **Role + Task + Context + Constraints + Format** 設計 Prompt（**自然語言** 或 **Markdown** 擇一）；**Format** 請明確要求「表格＋優先級欄位」或「條列＋每類一句回覆方向」。
...
**版本 B（五要素版）：**  
完整寫出 **Role、Task、Context、Constraints、Format**（可參考上方範例 4 的 **自然語言／Markdown** 任一結構）。
...
以下為**示意**：模擬「部門常見問答助理」貼在專案 Instructions 的寫法（實務上請替換成真實規章與聯絡窗口）。同樣提供 **自然語言**（整段口語指令，與 ROSES 五段內容等價）與 **Markdown**（便於直接貼入 Instructions）。摺疊區順序：**自然語言 → Markdown**。
...
- 同一套內容可寫成 **自然語言**（一段話交代完整條件）或 **Markdown**（易讀、易貼入對話），依工具與習慣擇一即可；進階再考慮是否改寫成 ROSES 長指令。
