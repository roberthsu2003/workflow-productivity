...
> 我要準備一段課堂用的自我介紹，但不知道 prompt 怎麼寫。請用一問一答引導我，問我幾個關鍵問題（例如：使用場景、語言、長度、語氣、一定要提到哪些關於自己的資訊），再依我的回答幫我產出完整的 prompt。我不需要上傳檔案，只要回答你的問題就好。
...
根據你的回答，AI 會組合出類似這樣的 prompt（你再把「關於我的事實」填進去或另開一則貼上即可）：
```
你是協助撰寫口語自我介紹的助手。使用者會提供幾項關於自己的事實（科系、興趣等），請依下列要求寫成一段可直接口說的開場自我介紹：
...
限制：不要捏造使用者未提供的事實；若資訊不足，請在輸出開頭用一句話提醒還缺哪一項。

格式：純文字一段，不要項目符號，方便直接念出。
```
