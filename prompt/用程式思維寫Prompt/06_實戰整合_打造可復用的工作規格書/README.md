...
本章我們將這四大要素全面整合為一套標準的 **ROSES 終極工作規格書架構**，並帶你親眼見證：**這份規格書如何一鍵轉換為未來的 AI Agent Skill！**

---

## 🎯 職場真實操作心法：別手打！用 Artifacts 側邊欄 5 步組裝終極規格

> 💡 **請永遠記住**：  
> 你不需要自己默背這份 50 行的終極模板。你真正要做的是：**在左側對話框發出自然語言指令，讓 AI 在右側 Artifacts 側邊欄一步步替你組裝出來！**

你在前面章節學到的每一個主題，正好對應一輪對話陳述句：

| 對話輪次 | 注入的程式陳述句概念 | 你在左側對話框說的簡單白話 | 右側 Artifacts 側邊欄的即時變化 |
|:---|:---|:---|:---|
| **Round 1** | **進入點 (Main Goal)** | *「請在右側 Artifacts 為我建立一份［某專業任務］的 ROSES 規格草案。」* | 生成基礎 R/O/S/E/S 骨架 (v1) |
| **Round 2** | **循序管線 (02 Pipeline)** | *「修改步驟：先分析現狀、再列 2 種方案大綱、最後依最優方案產出細節。」* | 步驟進化為 5 階段管線 (v2) |
| **Round 3** | **條件分支 (03 If-Else)** | *「在範圍中加入決策規則：如果預算不足就...如果資訊模糊就給保守假設。」* | 加入多情境分流防呆 (v3) |
| **Round 4** | **驗證迴圈 (04 While Loop)** | *「加入自我檢驗 Rubric 清單，規定產出後自檢，未達標最多修正 2 次。」* | 加入客觀自檢迴圈 (v4) |
| **Round 5** | **安全中斷 (05 Guardrail Break)** | *「加入停止條件：若遇到關鍵資料缺失或高風險，立即停產並輸出提問。」* | 升級為**終極可驗證規格書 (v5)** |

---

## 📄 終極工作規格架構：標準 ROSES 整合模板

這就是透過上述 5 步對話，AI 在右側 Artifacts 側邊欄為你淬鍊出的終極工作規格，適用於各類高複雜度的職場與技術任務：
