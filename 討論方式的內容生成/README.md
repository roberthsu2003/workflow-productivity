...
>
> 📌 **前置學習**：本單元所有範例皆採用 [AI 提示詞工程指南](../prompt/AI提示詞工程指南/README.md) 的 **ROSES 框架**或**4 要素**撰寫 prompt，請先熟悉該規範再進行練習。
...
| 原則 | 說明 |
|------|------|
| **依 ROSES 或 4 要素撰寫** | 角色、任務、步驟/背景、格式缺一不可，見 [AI 提示詞工程指南](../prompt/AI提示詞工程指南/README.md) |
...
**主題背景**：你有一個想分享的主題（例如：興趣、專長、學習心得），希望產出一篇結構完整的文章，並能與 AI 反覆討論修改。

<details>
<summary>📝 學生發想：先思考再下 prompt</summary>

在輸入 prompt 前，請先回答：
- 你的**主題**是什麼？
- 這篇文章的**讀者**是誰？（同學、老師、未來雇主？）
...
**主題背景**：你是社團幹部，每週要記錄社團會議內容。現有的會議記錄格式不統一，你希望產出一份「會議記錄範本」供未來使用，並能依實際需求逐步調整。
...
<details>
<summary>💬 第二步：圈選「待辦事項」表格後輸入（運用 4 要素）</summary>

```markdown
你是一位文書格式優化專家，請幫我修改這段「待辦事項」表格。
背景是我圈選的表格，需要更便於追蹤進度。
輸出格式請加上「優先級」欄位，並把格式改得更簡潔。
```

</details>

<details>
<summary>💬 第三步：繼續對話優化（運用 4 要素）</summary>

```markdown
你是一位會議記錄範本編輯，請幫我在文件中新增一個區塊。
背景是決議事項和待辦事項之間目前沒有銜接，需要補充。
輸出格式請在兩者之間加一個「下次會議時間」區塊。
```
...
**主題背景**：你發現自己常常拖延、時間不夠用，想整理一套「時間管理技巧」提醒自己，也方便分享給同學。
...
在輸入 prompt 前，請先發想：
- 你認為**最有效的 1～2 個**時間管理技巧是什麼？為什麼？
- 這篇文章你想**分享給誰**？他們最需要什麼？
- 你希望文章是**嚴肅的**還是**輕鬆的**語氣？
...
**主題背景**：延續練習一產出的「時間管理技巧」文章，你發現其中某一段寫得太正式、不夠貼近生活，結語也缺少行動呼籲，希望只修改這些部分而不動其他內容。

**目標**：學會圈選特定段落，請 AI 針對性修改，而不影響其他內容。

<details>
<summary>📝 學生發想：先思考再圈選修改</summary>
...
<details>
<summary>💬 建議 Prompt 1：修改單一技巧段落（運用 4 要素）</summary>

```markdown
你是一位貼近學生生活的文案編輯，請幫我修改這段內容。
背景是我圈選的「時間管理技巧」其中一段，目前寫得太正式。
輸出格式請改得更口語、更貼近大學生的生活經驗，並加一個具體例子。
```
...
<details>
<summary>💬 建議 Prompt 2：修改結語（運用 4 要素）</summary>

```markdown
你是一位擅長行動呼籲的文案編輯，請幫我強化這段結語。
背景是我圈選的結語段落，目前缺少行動呼籲。
輸出格式請加上一句鼓勵讀者「今天就開始行動」的呼籲。
```
...
**主題背景**：新學期開始，你想為自己訂一份「學期讀書計畫」，但不知道從何開始。你希望 AI 先產出一份範本，再透過多輪對話，逐步加入你需要的元素（如 SMART 目標、完成度追蹤、複習策略等），直到打磨成適合自己的版本。

**目標**：透過至少 **5 輪對話**，將一份草稿打磨成完整文件。

<details>
<summary>📝 學生發想：先規劃再產出</summary>

在產生初稿前，請先發想：
- 你這學期最想**達成的一個目標**是什麼？
- 你習慣用**表格**還是**條列**來規劃？為什麼？
- 你聽過**SMART 原則**或**費曼學習法**嗎？想加入哪些？
...
2. 進行至少 5 輪修改，每輪先思考「這份計畫還缺什麼」，再下指令（可參考下方範例，運用 4 要素）

<details>
<summary>💬 建議 Prompt：5 輪修改範例（運用 4 要素）</summary>

| 輪次 | 建議指令 |
|------|----------|
| 第 1 輪 | `你是一位學習策略專家，請幫我補充目標設定區塊。背景是目前的目標設定較簡略。輸出格式請加上 SMART 原則的說明。` |
| 第 2 輪 | `你是一位計畫表設計師，請幫我優化每週計畫表格。背景是表格需要追蹤完成度。輸出格式請加上「完成度」欄位。` |
| 第 3 輪 | `你是一位學習方法顧問，請幫我改寫複習策略區塊。背景是希望加入有效的學習法。輸出格式請改用條列式，並加上費曼學習法的介紹。` |
| 第 4 輪 | `你是一位青少年讀物編輯，請幫我調整整份文件的語氣。背景是讀者為高中生。輸出格式請改得更活潑，適合高中生閱讀。` |
| 第 5 輪 | `你是一位學習教練，請幫我在文件最後新增一個區塊。背景是計畫需要定期檢視。輸出格式請加上一個「月底回顧」的區塊。` |
...
| 想做的事 | 建議 prompt 關鍵字 |
|----------|-------------------|
| **撰寫 prompt 規範** | 所有 prompt 請依 [ROSES 框架](../prompt/AI提示詞工程指南/README.md#roses-提示框架) 或 [4 要素](../prompt/AI提示詞工程指南/README.md#簡化版4-個核心要素) 撰寫 |
| 產生可編輯的 Markdown 文章 | 角色＋任務＋步驟＋格式＋「在 Canvas/畫布/Artifact 中顯示」 |
| 產生可編輯的程式碼 | 角色＋「用 [語言] 寫 [功能]」＋背景＋「在編輯器中顯示，方便我修改」 |
| 沒有跳出編輯器 | 明確加上「使用畫布」「在 Canvas 中顯示」「以 Artifact 顯示」 |
| 只修改某一段 | 先**圈選**該段，再用 4 要素（角色、任務、背景、格式）下指令 |
