...
| **`gpt-oss:20b` 產出範例**（對照用） | [`gpt-oss-20b_完成結果範例.md`](./資料來源/gpt-oss-20b_完成結果範例.md) |

上表最後一列（**`gpt-oss:20b` 產出範例**）為以**手動合併逐字稿**＋本頁**會議文件用提示詞**餵入模型後之存檔，方便對照四區格式；**不代表正式會議結論**，仍須人工核稿。
...
4. 產出風格可對照範例：[`資料來源/gpt-oss-20b_完成結果範例.md`](./資料來源/gpt-oss-20b_完成結果範例.md)
