...
| 檔案名稱 | 類型 | 說明 |
| :--- | :--- | :--- |
| [`跨平台成效偽數據.md`](./跨平台成效偽數據.md) | 核心素材 | 輕量化 Markdown 表格版本（極省 Token），含跨平台指標對照定義與 8 月異常商業痛點說明。 |
| [`3-1_白話自然語言發想提示詞.txt`](./3-1_白話自然語言發想提示詞.txt) | 提示詞 (輸入) | 學生第一步發送給 AI 的極簡白話指令（已直接內嵌輕量 CSV 數據文字，可一鍵複製貼入）。 |
...
```mermaid
flowchart TD
    A["階段一：準備偽數據<br/>(開啟 跨平台成效偽數據.md 或使用 3-1 內嵌數據)"] --> B["階段二：白話人機協商<br/>(輸入 3-1 提示詞提出 8 月異常困惑)"]
...
### 階段一：準備省 Token 數據
打開 [`跨平台成效偽數據.md`](./跨平台成效偽數據.md)（或直接參考 [`3-1_白話自然語言發想提示詞.txt`](./3-1_白話自然語言發想提示詞.txt) 中已內嵌的 CSV 純文字）：
...
請學生在任何 AI 對話框直接貼上以下白話內容（已內嵌 Excel 轉出的省 Token CSV 數據，點擊右上角即可一鍵複製）：
...
執行階段二後，AI 會自動提煉出高階結構化提示詞（如下）。學生可直接**整段複製**，貼入 ChatGPT 的 **Work 模式** 執行：
...
[← 返回實戰範例庫總覽](../README.md) ｜ [前往範例：社群行銷素材多格式產出](../社群行銷素材多格式產出/README.md) ｜ [前往範例：便利超商門市排班與規則檢核](../便利超商門市排班與規則檢核/README.md)
