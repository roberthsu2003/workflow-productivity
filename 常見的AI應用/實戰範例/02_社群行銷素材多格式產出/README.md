...
4. 短影音 Reels/Shorts 分鏡腳本（符合 Google Flow / Veo 影片規格：16 秒快節奏版 4 鏡頭 × 4s、30 秒手作版 5 鏡頭 × 6s，含 I2V/T2V 模式、中文畫面運鏡、口播詞、字卡與 Google Flow 專用英文提示詞 Prompt）
...
4. 短影音直式分鏡腳本 2 版（符合 Google Flow / Veo 規範：16 秒快節奏版採 4 鏡頭 × 4s，30 秒手作說明版採 5 鏡頭 × 6s，含 I2V/T2V 模式、中文畫面運鏡、口播詞、字卡與 Google Flow 專用英文提示詞 Prompt）
...
3. 短影音分鏡嚴格符合 Google Flow / Veo 影片生成規格：
   - 單鏡頭時長嚴格採用 Google Flow 標準單段時長（4 秒或 6 秒模組化），不可出現隨意不相容秒數。
   - 16 秒版規劃 4 鏡頭（各 4s）；30 秒版規劃 5 鏡頭（各 6s）。
   - 首鏡頭（Shot 1）設定為 Image-to-Video (I2V)，以前述生成之 9:16 直式宣傳圖為首幀；後續鏡頭設定為 T2V 或 Extend。
   - 表格欄位必須包含：鏡頭 (Shot)、時長、生成模式 (I2V/T2V)、畫面構圖與運鏡描述、旁白口播詞 (Voiceover)、音效與字卡 (Text / Audio)、Google Flow 專用英文提示詞 (Prompt)。
...
- 區塊三：【官網文章主題切角與標題庫】
- 區塊四：【短影音 Reels / Shorts 直式分鏡腳本庫（Google Flow / Veo 規範版：16s / 30s）】
- 區塊五：【主視覺封面短標題建議】
```
</details>
...
```text
 ...
 │                           │                                          │
 │ 💬 輸入微調指令：         │ 🎬 區塊四：短影音分鏡腳本庫（Google Flow）│
 │ 「請將 16 秒短影音的 Shot│ | 鏡頭 | 時長 | 模式 | 畫面運鏡 | 提示詞 |│
 │  1 英文 Prompt 加入柔和金 │ |------|------|------|----------|--------|│
 │  黃晨光與慢動作運鏡。」   │ | Shot1| 4s   | I2V  |【反白】  |【圈選】|│
 │                           │ | Shot2| 4s   | T2V  | 微距平移 | ...    |│
 │                           │                                          │
 └───────────────────────────┴──────────────────────────────────────────┘
```
...
| 檔案名稱 | 類型 | 說明 |
| :--- | :---: | :--- |
| 🏆 [**`秋季新品_焦糖焙香燕麥拿鐵_社群全格式素材包.md`**](./秋季新品_焦糖焙香燕麥拿鐵_社群全格式素材包.md) | **完成作品** | **最終文案成果檔**：包含 IG、FB、官網標題、符合 Google Flow (4s/6s) 規範之短影音 16s/30s 分鏡表與封面短標。 |
| 📱 [**`新品上市宣傳圖_IG貼文_1080x1080.jpg`**](./新品上市宣傳圖_IG貼文_1080x1080.jpg) | **規格圖片** | **Instagram 貼文規範**：1:1 正方形（1080 × 1080 px）。 |
| 📱 [**`新品上市宣傳圖_IG限動Reels_1080x1920.jpg`**](./新品上市宣傳圖_IG限動Reels_1080x1920.jpg) | **規格圖片** | **IG 限動 / Reels 規範**：9:16 直式滿版（1080 × 1920 px，含背景高斯模糊）。 |
...
### 4. Google Flow / Veo 短影音生成的模組化工法
Google Flow（Veo 模型）對於生成時長有嚴格的原生段落規範（4 秒 / 6 秒 / 8 秒）。
- **拒絕隨意時長**：傳統影音腳本常寫「0-3 秒」、「3-7 秒」，但 AI 影片工具無法精確裁切此類隨意秒數。本工作流將短影音嚴格模組化為「16 秒版（4s × 4）」與「30 秒版（6s × 5）」，每段皆能獨立一鍵貼入 Google Flow 生成。
- **I2V 首幀鎖定**：第一鏡頭（Shot 1）直接以上一步生成的 9:16 宣傳圖作為首幀參考圖（Image-to-Video），能 100% 確保品牌產品在動態影片中不變形、拉花特徵不走樣。
- **英文運鏡提示詞直出**：Veo 核心模型對英文影視語彙（如 `push-in` 推進鏡頭、`macro sliding shot` 微距平移、`shallow depth of field` 淺景深）反應最靈敏。提示詞直接為每鏡頭生成雙語欄位，學員複製英文 Prompt 即可在 Google Flow 完美出片！
