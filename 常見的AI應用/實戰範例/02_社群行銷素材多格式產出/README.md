...
flowchart TD
    ...
    S5 --> S6["【關卡 6】Google Flow 直式短影音生成<br/>(手動設定 9:16 直式 + 圖片先行 + AI 提示詞)"]
...
### 🟢 關卡 6：Google Flow 直式短影音實戰生成（最新版介面：手動直式設定 ＋ 圖片先行法）

* 📌 **操作背景**：在取得專案產出的 9:16 直式規格圖片與分鏡 Prompt 後，學員將使用最新版 **Google Flow** 生成符合 **Instagram Reels** 與 **YouTube Shorts** 的 4 秒 / 6 秒 / 8 秒直式短影音。
* ⚠️ **最新版本重要注意事項**：
  * **Google Flow 預設的影片輸出比例為橫式（16:9）**。因為我們要製作的是手機直式短影音，**必須先進入「代理設定」手動將輸出切換為 `9:16` 直式**，否則生成出來會變成橫向影片！
  * **核心工法「圖片先行」**：切忌只輸入文字描述！**務必先將關卡 5 產出的 9:16 宣傳圖加入對話框**，再貼上 AI 生成的提示詞，才能 100% 鎖定陶杯拉花、手作質感與光影特徵！

```mermaid
flowchart LR
    A["進入 Google Flow<br/>右上角 ⚙️ 代理設定"] --> B["【關鍵】手動設定<br/>影片長寬比為 9:16 直式"]
    B --> C["對話框點選 ＋ 號<br/>先加入 9:16 宣傳圖片"]
    C --> D["貼入 AI 生成分鏡<br/>英文提示詞 Prompt"]
    D --> E["點擊 ➜ 送出生成<br/>(支援 4s / 6s / 8s 規格)"]
    E --> F["點擊 Extend 接續延伸<br/>串聯 Shot 2～Shot 4 分鏡"]
```
...
2. **步驟 2：在對話框「先行加入圖片」（首幀參考圖，鎖定視覺一致性）**
   * 回到右側工作階段主介面，找到下方的 **「你想製作什麼內容？」** 輸入對話框。
   * 點擊對話框左側的 **「`+`」** 按鈕（或從左側功能選單「上傳項目」拖入）。
   * 選擇加入在關卡 5 由 Python 產出的：[`新品上市宣傳圖_IG限動Reels_1080x1920.jpg`](./新品上市宣傳圖_IG限動Reels_1080x1920.jpg)。
   * 💡 **為什麼要圖片先行？** 純文字生成容易讓咖啡陶杯或拉花在每一格中變形，先加入 9:16 宣傳圖能作為精確的視覺錨點（Image-to-Video）。

3. **步驟 3：貼入 AI 生成的英文提示詞（Prompt）並指定秒數**
   * 在已經帶有圖片縮圖的輸入框內，複製貼入 [`秋季新品_焦糖焙香燕麥拿鐵_社群全格式素材包.md`](./秋季新品_焦糖焙香燕麥拿鐵_社群全格式素材包.md) 區塊四中 Shot 1 的 **Google Flow 專用英文提示詞**。例如：
     ```text
     Cinematic close-up push-in shot of hot latte in a handcrafted ceramic mug, tulip latte art, delicate steam rising, soft morning window light, warm autumn tones, photorealistic 8k, slow motion. --ar 9:16
     ```
   * 點擊右下角 **「➜ 送出」** 箭頭，Google Flow 即會根據 9:16 設定與首圖生成一段極致穩定的短影音！

4. **步驟 4：善用 Extend（接續延伸）完成多鏡頭模組**
   * 當第一段影片生成完畢後，點擊該影片右下角的 **Extend（接續延伸）** 功能，或繼續在對話框中送出下一段鏡頭的英文 Prompt（支援 4 秒、6 秒或 8 秒標準模組）。
   * 將生成的各段 9:16 影片下載至手機，匯入剪輯軟體（如 CapCut / 剪映）加上素材包中準備好的 **旁白口播詞（VO）** 與 **字卡**，即可直接發布至 Instagram Reels 與 YouTube Shorts！

* 🔍 **成果檢查點**：
  * [ ] 檢查右上角代理設定中，影片輸出比例已確實點選為 **`9:16`**。
  * [ ] 生成前**已先將 9:16 宣傳圖片加入輸入框**，才發送文字 Prompt。
  * [ ] 生成的影片呈現直式滿版（1080×1920），陶杯拉花外觀完整且運鏡平穩！
...
* 🔍 **最終驗收檢查點**：
  * [ ] 產出的 5 大區塊文案是否兼顧了「生活感」、「研發故事」與「符合 4s/6s/8s 的短影音分鏡表格」？
